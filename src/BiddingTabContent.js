...
function BiddingTabContent({ data, onPlaceBidClick }) {
    return (
        <>
            ...
            <h3 style={{ textAlign: 'center' }}>Click on a row to see more details about each item.</h3>
            <Accordion>
                {data.biddingItems.map(biddingItem => <Card key={biddingItem.key}>
                    <Accordion.Toggle as={Card.Header} eventKey={biddingItem.key}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}><div>{biddingItem.title}</div><div>Current highest bid: {`$${biddingItem.highestBid}.00`}</div></div>
                    </Accordion.Toggle>
